...
module.exports = function pluginTests(test, plugin) {
  ...
  if (plugin.hasOwnProperty('validation')) {
  ...
  test('Inputs configured correctly', t => {
    ...
    inputs.map(input => {
      ...
      if (imp.hasOwnProperty('options')) {
        t.true(Array.isArray(imp.options), 'Options is an array');
  ...
  test('HTML is written correctly, includes required attributes, and is render-able', t => {
    ...
    inputTests = inputs.map(input => {
      return htmls.map(tst => {
        ...
        htmlRegex = htmlRegex || new RegExp(`<${tag}[a-zA-Z\\d\\=\\"\\{\\}\\s\\-\\_\\|\\.\\(\\)\\%\\']*(${thisTest.prop}=['"]\\s*{{\\s*${input}.${thisTest.key}\\s*}}\\s*['"])`);
    ...
    optionsTests = inputs.map(input => {
      return htmls.map(tst => {
        ...
        if (plugin.inputs[input].type === 'select') {
          ...
          const iterator = plugin.html.match(new RegExp(`{%\\s*?for\\s*([\\w\S]*)\\s*?in\\s*?${input}`));

          if ((thisTest.prop === 'id') || (thisTest.prop === 'for')) {
            key = `${input}.${thisTest.key}}}--{{loop.index`;
          }
          ...
          htmlRegex = new RegExp(`<${tag}[a-zA-Z\\d\\=\\"\\{\\}\\s\\-\\_\\|\\.\\(\\)\\%\\']*(${thisTest.prop}=['"]\\s*{{\\s*${key}\\s*}}\\s*['"])`);
        }
